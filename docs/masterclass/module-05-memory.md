...
title: "模組 5: 持久記憶與個人化"
sidebar_position: 6
description: "深入理解 OpenClaw 的 Memory System — Write-Ahead Logging、Markdown Compaction、Context Window 管理與記憶個人化"
keywords: [OpenClaw, Memory, WAL, Markdown Compaction, 記憶, Context Window, 個人化]
---
...
- 說明 OpenClaw Memory System 的三大核心元件
- 理解 Write-Ahead Logging (WAL) 的運作機制與資料安全保證
- 說明 Markdown Compaction 如何將碎片記憶壓縮為結構化摘要
- 設定 Context Window 管理策略
- 配置記憶保留策略，實現 Agent 個人化
- 排除常見的記憶系統問題

:::info 前置條件
請先完成 [模組 1: 基礎架構](./module-01-foundations)，理解 Memory System 在四層架構中的位置。
:::
...
| 元件 | 職責 | 類比 |
|------|------|------|
| **WAL Engine** | 所有記憶變更先寫入 WAL，確保資料不遺失 | 資料庫的 Transaction Log |
| **Markdown Compaction** | 定期將 WAL 中的碎片記憶壓縮為結構化摘要 | 資料庫的 Compaction / Vacuum |
| **Context Window Manager** | 智慧選擇相關記憶，注入 LLM 的 Context Window | 搜尋引擎的 Ranking |

---

## Write-Ahead Logging (WAL)

WAL 是記憶系統的基礎安全機制。所有記憶操作（新增、修改、刪除）都必須先寫入 WAL 檔案，確保即使系統意外當機，記憶資料也不會遺失。

### WAL 運作流程

```
記憶操作                WAL                     實際記憶
(store)               (預寫日誌)                (compacted)
   │                     │                        │
   │  1. 寫入 WAL        │                        │
   ├────────────────────▶│                        │
   │                     │                        │
...
WAL 檔案儲存在 `~/.openclaw/memory/wal/` 目錄下：
...
```json
{
  ...
  "channel": "default",
  "type": "conversation",
  "content": "使用者偏好使用繁體中文回應，技術術語保留英文",
  "importance": 0.85,
...
WAL 中的記憶條目是碎片化的 — 每次對話可能產生數十條記憶片段。**Markdown Compaction** 定期將這些碎片壓縮為結構化的 Markdown 文件，減少儲存空間並提升檢索效率。
...
壓縮後的記憶儲存在 `~/.openclaw/memory/compacted/` 目錄下：

```
~/.openclaw/memory/compacted/
├── user-profile.md           # 使用者個人資料與偏好
...
min_wal_entries = 20             # WAL 條目少於此數不觸發壓縮
importance_threshold = 0.3       # 低於此重要性的記憶在壓縮時可能被丟棄
max_compacted_size = "10MB"      # 壓縮後文件的最大大小
...
- **`manual`（手動）**：由使用者手動控制注入哪些記憶

### 注入流程視覺化

```
使用者訊息: "繼續昨天的 Project X 前端開發"

Context Window Manager 執行：
┌─────────────────────────────────────────────┐
│ 1. 語意搜尋 "Project X 前端開發"             │
│    → 找到 12 筆相關記憶                      │
│                                             │
│ 2. 加權評分排序                              │
│    → [user-profile: 0.92]                   │
...
| 類型 | 用途 | 保留策略 | 範例 |
|------|------|----------|------|
| `preference` | 使用者偏好 | 永久 | 語言偏好、介面設定 |
| `fact` | 已知事實 | 永久 | 公司名稱、技術堆疊 |
| `conversation` | 對話摘要 | 依設定 | 昨天討論了什麼 |
| `task` | 任務記錄 | 完成後 30 天 | 待辦事項、進度 |
| `ephemeral` | 臨時記憶 | Session 結束 | 當前操作的暫存資訊 |

### 最佳實踐

:::tip 記憶管理最佳實踐

1. **定期審視記憶**：每週使用 `openclaw memory list` 檢查記憶內容，刪除過時或不正確的記憶。

2. **善用標籤**：為重要記憶加上有意義的標籤，幫助 Context Window Manager 更精準地檢索。

3. **設定合理的保留期限**：不要永久保留所有記憶。`conversation` 類型建議保留 90 天，`task` 類型保留 30 天。

4. **注意隱私**：敏感資訊（如密碼、API Key）不應存入記憶。如果不小心存入，立即使用 `openclaw memory delete` 移除。

5. **監控記憶大小**：記憶過多會影響 Compaction 效能。使用 `openclaw memory stats` 定期監控。
:::
...
**解法**：Compaction 需要呼叫 LLM 來產生摘要。確認 LLM Provider 連線正常。
```bash
# 測試 LLM 連線
openclaw test provider

# 使用離線 Compaction（不產生摘要，只合併）
...
1. **記憶持久性測試**：在 OpenClaw 中進行一段包含多個主題的長對話（至少 20 則訊息），然後重啟系統，驗證 Agent 在新 Session 中能回想起多少資訊。記錄哪些記憶被保留、哪些被遺忘，分析原因。

2. **Compaction 觀察**：將 `compaction_interval` 設為 300 秒（5 分鐘），連續與 Agent 對話 30 分鐘，然後檢查 `compacted/` 目錄下產生的 Markdown 文件。分析壓縮的品質。

3. **Context Window 最佳化**：嘗試三種不同的 Context Window 策略（`adaptive`、`fixed`、`manual`），在相同的對話場景下比較 Agent 的回應品質。

4. **記憶清理**：使用 `openclaw memory list` 和 `openclaw memory search` 找出所有過時或不正確的記憶，執行清理，然後驗證 Agent 不再引用這些已刪除的記憶。
...
1. **B** — WAL 是 Write-Ahead Logging 的縮寫，是一種先將變更寫入日誌、再實際修改資料的技術。
2. **B** — Markdown Compaction 將散亂的 WAL 記憶片段壓縮為結構化的 Markdown 文件，提升儲存效率與檢索品質。
3. **C** — 相關性（Relevance）的權重為 0.4，是四個因素中最高的。
4. **C** — `preference`（使用者偏好）和 `fact`（已知事實）的預設保留策略都是永久。
