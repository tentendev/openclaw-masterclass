---
title: "模組 7: 瀏覽器自動化與網頁爬取"
sidebar_position: 8
description: "掌握 OpenClaw 的 Headless Chromium 與 Puppeteer 整合，建構強大的網頁爬取與瀏覽器自動化 Agent"
keywords: [OpenClaw, browser, Puppeteer, Chromium, headless, web scraping, 瀏覽器自動化, 爬蟲]
---
...
OpenClaw 透過內建的 Headless Chromium 引擎與 Puppeteer API，讓 Agent 能夠像人類一樣操作瀏覽器：
...
| 模式 | 說明 | 適用情境 |
|------|------|---------|
| **Headless** | 無 GUI，背景執行 | 伺服器環境、排程爬取、CI/CD |
| **Headed** | 有 GUI 顯示 | 開發除錯、需要人工介入的流程 |
| **Headless (New)** | Chrome 113+ 新 Headless 模式 | 需要更完整瀏覽器行為模擬 |
...
- Skill 生態系中大多數瀏覽器 Skill 基於 Puppeteer
- 較低的記憶體佔用（僅需 Chromium，不需多瀏覽器引擎）

## 實作教學

### 步驟一：安裝與設定

確認 OpenClaw 的瀏覽器模組已啟用：
...
:::danger 安全性警告
`--no-sandbox` 參數會停用 Chromium 的沙箱保護。在正式環境中，強烈建議使用 Podman 容器來提供外層隔離，而非停用沙箱。詳見 [模組 9: 安全性](./module-09-security)。
:::

### 步驟二：基本瀏覽器操作

透過 Agent 的自然語言指令操作瀏覽器：

```
使用者: 幫我打開 https://example.com 並截圖
Agent:  [啟動 Headless Chromium]
        [導航至 example.com]
        [截圖完成，已儲存至 /tmp/screenshot-2026-03-20.png]
```
...
這是一個完整的實戰專案 — 監控電商平台的商品價格，價格低於門檻時自動通知。
...
:::tip 結合模組 6 排程
價格監控最佳實踐是結合[模組 6: Cron Jobs](./module-06-automation) 中的排程機制，每隔數小時自動檢查一次。避免頻率過高（每分鐘），以免被電商平台封鎖 IP。
:::
...
:::caution 憑證安全
永遠不要將帳號密碼寫死在 Skill 程式碼中。使用 OpenClaw 的環境變數或 Secret Manager：
```bash
...
2. 設定 `max_concurrent_pages` 上限
3. 在 Docker/Podman 中設定記憶體限制
...
### 練習 1：基礎截圖
建立一個 Skill，接受 URL 清單，為每個頁面擷取完整截圖，並組成 PDF 報告。

### 練習 2：新聞彙整
撰寫一個每日新聞爬取 Agent，從 3 個科技新聞網站擷取今日頭條，並整理成 Markdown 格式的摘要。
...
1. **`waitUntil: 'networkidle2'` 代表什麼意思？**
   ...
   <details><summary>查看答案</summary>B) `networkidle2` 表示在 500 毫秒內不超過 2 個進行中的網路連線，適用於大多數頁面載入情境。</details>

2. **為什麼 Headless Chromium 需要 `--no-sandbox` 參數？**
   - A) 提升效能
   - B) 在 Docker 容器或非 root 環境中，Linux 的 user namespace sandbox 可能無法使用
   - C) 啟用更多功能
   - D) 減少記憶體使用

   <details><summary>查看答案</summary>B) 在容器化環境中，Linux 的 sandbox 機制可能與容器的隔離層衝突。但這會降低安全性，建議搭配 Podman 容器使用。</details>

3. **價格監控 Agent 的最佳檢查頻率是？**
   - A) 每分鐘
   - B) 每 2-4 小時
   - C) 每天一次
   - D) 即時監控

   <details><summary>查看答案</summary>B) 每 2-4 小時是較佳的平衡點。太頻繁容易被封鎖 IP，太少則可能錯過限時優惠。可根據商品特性調整。</details>
...
4. **處理 SPA 動態內容時，應該使用哪個方法？**
   ...
   <details><summary>查看答案</summary>B) SPA 的內容是由 JavaScript 動態渲染的，必須等待目標元素實際出現在 DOM 中後才擷取。</details>

## 建議下一步

- [模組 6: Cron Jobs / Heartbeat](./module-06-automation) — 將爬蟲排程為自動化任務
- [模組 8: 多 Agent 架構](./module-08-multi-agent) — 讓多個 Agent 分工爬取不同網站
- [模組 9: 安全性](./module-09-security) — 了解瀏覽器自動化的安全最佳實踐
