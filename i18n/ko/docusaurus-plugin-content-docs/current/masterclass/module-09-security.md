...
title: "모듈 9: 보안"
sidebar_position: 10
description: "OpenClaw의 보안 위험, CVE-2026-25253 취약점, ClawHavoc 공격 사건 심층 이해 및 완전한 보안 강화 체크리스트"
---
...
- 설치了 `code-runner` 或類似可실행程式碼的 Skill
...
Podman 預設以 rootless 模式運行，不需要 daemon，且支援 `--userns=keep-id`。這比 Docker 更適合安全敏感的배포。詳見[模組 10: 正式環境배포](./module-10-production)。
:::
...
1. **CVE-2026-25253 的 CVSS 評分是多少？**
   ...
   <details><summary>정답 확인</summary>C) CVSS 9.8，屬於 Critical 等級。這是因為該취약점允許未검증的遠端程式碼실행。</details>
...
2. **以下哪個綁定方式是安全的？**
   ...
   <details><summary>정답 확인</summary>C) `127.0.0.1:18789` 只監聽 loopback 介面，僅本機可存取。其他選項都會監聽所有網路介面。</details>
...
3. **ClawHavoc 攻擊的主要入侵方式是什麼？**
   ...
   <details><summary>정답 확인</summary>B) ClawHavoc 利用 Shodan 掃描暴露在公網上的 OpenClaw 實例，並透過 CVE-2026-25253 취약점進行自動化入侵。</details>

4. **추천하는 이유 Podman 而非 Docker 來배포 OpenClaw？**
   - A) Podman 比較快
   - B) Podman 預設 rootless、無 daemon、更適合安全敏感環境
   - C) Docker 不支援 OpenClaw
   - D) Podman 免費而 Docker 收費

   <details><summary>정답 확인</summary>B) Podman 預設以非 root 사용자運行，不需要長駐的 daemon 程序，減少攻擊面。</details>

## 다음 단계

- [模組 10: 正式環境배포](./module-10-production) — 在安全的前提下배포到正式環境
- [模組 8: 多 Agent 架構](./module-08-multi-agent) — 了解多 Agent 環境中的額外보안 고려사항
- [模組 12: 엔터프라이즈級應用](./module-12-enterprise) — 엔터프라이즈級的安全컴플라이언스要求
