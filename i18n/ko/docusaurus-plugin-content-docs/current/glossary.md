...
| Auth Token | 인증 토큰 | 认证 Token | 認證 Token | 認証トークン | Gateway API의 인증 자격 증명 |
...
| Browser Use | 브라우저 사용 | 浏览器操控 | 瀏覽器操控 | ブラウザ操作 | Agent가 웹 브라우저를 조작하는 스킬 |

### C

| 영어 | 한국어 | 简体中文 | 繁體中文 | 日本語 | 설명 |
|------|--------|---------|---------|--------|------|
| Channel | 채널 | 频道 / 通讯平台 | 頻道 / 通訊平台 | チャンネル | Agent가 연결하는 메신저 플랫폼 (Telegram 등) |
| Channel Adapter | 채널 어댑터 | 频道适配器 | 頻道轉接器 | チャンネルアダプター | 특정 플랫폼 프로토콜을 처리하는 컴포넌트 |
| ClawHavoc | ClawHavoc 사건 | ClawHavoc 事件 | ClawHavoc 事件 | ClawHavoc事件 | 2025년 말 악성 스킬 공급망 공격 |
| ClawHub | ClawHub 스킬 마켓 | ClawHub 技能市场 | ClawHub 技能市集 | ClawHubスキルマーケット | OpenClaw의 스킬 공유 플랫폼 |
| Compaction | 압축 | 压缩 | 壓縮 | コンパクション | WAL을 구조화된 Markdown으로 압축하는 과정 |
| Container | 컨테이너 | 容器 | 容器 | コンテナ | 스킬 실행을 위한 격리 환경 (Podman/Docker) |
| Context Window | 컨텍스트 윈도우 | 上下文窗口 | 上下文窗口 | コンテキストウィンドウ | LLM이 처리할 수 있는 최대 토큰 수 |
| CVE | CVE | CVE（通用漏洞披露） | CVE（通用漏洞揭露） | CVE（共通脆弱性識別子） | 공개된 보안 취약점 식별 코드 |
...
| Execution Layer | 실행 레이어 | 执行层 | 執行層 | 実行レイヤー | 4계층: 스킬이 샌드박스에서 실행 |
...
| Gateway | 게이트웨이 | 网关 / Gateway | 閘道 / Gateway | ゲートウェイ | 1계층: 메시지 통합 수신 진입점 (port 18789) |
...
| 영어 | 한국어 | 简体中文 | 繁體中文 | 日本語 | 설명 |
|------|--------|---------|---------|--------|------|
| Manifest | 매니페스트 | 技能清单文件 | 技能宣告檔 | マニフェスト | 스킬의 메타데이터와 권한 선언 |
...
| Permission | 권한 | 权限 | 權限 | パーミッション | 스킬이 접근 허용된 리소스 범위 |
...
| Reasoning Layer | 추론 레이어 | 推理层 | 推理層 | 推論レイヤー | 2계층: 의도 인식과 응답 생성 처리 |
...
| Skill | 스킬 | 技能 | 技能 | スキル | Agent가 실행할 수 있는 확장 기능 모듈 |
| SOUL.md | SOUL.md (성격 설정) | SOUL.md（人格配置文件） | SOUL.md（人格設定檔） | SOUL.md（パーソナリティ設定） | Agent 성격과 동작을 정의하는 Markdown 파일 |
...
| WAL | WAL (선행 기록 로그) | WAL（预写日志） | WAL（預寫日誌） | WAL（先行書き込みログ） | Write-Ahead Log, 대화를 실시간으로 기록하는 메커니즘 |
