...
title: "AI 모델 선택"
sidebar_position: 4
description: "OpenClaw가 지원하는 주요 LLM 모델 비교 — Claude, GPT, Gemini, DeepSeek, Ollama, GLM-5, KIMI K2.5의 비용, 속도, 품질 및 프라이버시 트레이드오프"
---

# 選擇 AI 模型

OpenClaw 支援多家 LLM 提供者，你可以根據需求在不同場景中使用不同模型，甚至설정自動切換機制。本篇將幫助你做出最佳選擇。
...
**優勢**：推理品質極高、답변自然、安全護欄完善、超長上下文（1M tokens）
**劣勢**：Opus 價格較高、部分地區지연較高
...
Gemini 3 Pro 的多模態能力（圖片、영상理解）在 OpenClaw 中特別有用。
...
:::warning DeepSeek 프라이버시 주의사항
DeepSeek 是中國公司，其 API 服務的데이터處理受中國法規管轄。如果你處理敏感個資或엔터프라이즈機密，請審慎評估是否使用。可改用 Ollama 在本機運行 DeepSeek 開源模型版本。
:::
...
| **速度** | 視硬體而定 | 通常較快 |
| **品質** | 開源模型略遜 | 頂級閉源模型 |
| **가용성** | 永遠可用 | 依賴網路與 API 狀態 |
| **維護** | 需自行업데이트模型 | 提供者自動업데이트 |

:::info 混合模式推薦
最佳實踐是설정混合模式：日常對話使用本機模型（零成本、高隱私），需要高品質推理時自動切換到雲端 API。
...
routing:
  ...
  escalation:
    ...
    trigger:
      ...
      - explicit_request: true  # 사용자明確要求時
      - skill_requirement: true # 스킬要求雲端模型時
```
:::
...
- [SOUL.md 人格설정](./soul-md-config.md) — 設計你的 AI 代理的性格與行為
- [連接커뮤니케이션平台](./connect-channels.md) — 如果你還沒連接任何平台
