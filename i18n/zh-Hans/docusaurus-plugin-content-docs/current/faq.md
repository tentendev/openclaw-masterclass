...
- **云端 LLM**（Claude、GPT 等）：你的对话会传送到提供商的服务器进行处理
...
**A：** SOUL.md 是一个 Markdown 文件，定义了你的 Agent 的人格、行为规则、安全边界和日常任务。它是 Agent 的"灵魂"——决定了 Agent 如何思考和行动。
